...
def main():
    with sync_playwright() as p:
        page_url = rf"https://en.yellowpages.uz//rubric/private-clinics/tashkent?pagenumber=3&pagesize=100"
        ...
        for i in _count:
            phone_link_element = i.locator(
                '//a[contains(@onclick, "showPhonePopup")]'
            ).first  # Find the link with the onclick
            ...
            _dict = {
                "name": i.locator(
                    '//a[@class="organizationName blueText"]'
                ).inner_text(),
                "address": i.locator('//p[@class="address"]')
                .inner_text()
                .replace("Address: ", ""),
