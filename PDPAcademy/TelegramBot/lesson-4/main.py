...
async def age_handler(message: Message, state: FSMContext) -> None:
    ...
    await message.answer('Enter your phone number with the button.', reply_markup=rkb.as_markup(resize_keyboard=True))
...
async def phone_number_handler(message: Message, state: FSMContext) -> None:
    ...
    await message.answer('Enter your location with the button.', reply_markup=rkb.as_markup(resize_keyboard=True))
